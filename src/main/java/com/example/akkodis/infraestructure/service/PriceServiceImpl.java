...
public class PriceServiceImpl implements PriceService {
    ...
    public PricesEntity getPriceByFilters(String date, int productId, int brandId) {
        LocalDateTime targetDate = Tools.convertToDate(date);

        return repository.findByBrandIdAndProductId(productId, brandId).stream()
            .filter(p -> Tools.convertToDate(p.getEnd_Date()).isAfter(targetDate))
            .max(Comparator.comparing(p -> Tools.convertToDate(p.getStart_Date())))
            .orElse(null);
    }
}
